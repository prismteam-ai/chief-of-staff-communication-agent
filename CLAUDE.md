...
5. **Telegram MTProto** (Telethon, personal account) — BUILT + wired; live activation blocked by Telegram's server-side login validation (in-app session approval + relayed-code invalidation), not our code. See channel-scope note below.
6. **Demo tenant real data** — DONE 2026-07-09. Demo Gmail is hard to create now (Arthur), so used a 2nd real Zoho: `chiefofstaff.demo@zohomail.com` (IMAP enabled + app-password) connected to the demo tenant via the UI; seeded 5 real professional emails (one real external sender `democos@zohomail.com` as "Priya Nair", distinct threads) + Zoho's welcome. Then **purged the 137 leftover fixture/persona DATA rows** that survived the code retirement (see Learned the hard way). Demo tenant is now real-only: **6 messages, all `imap:%`**, triaged into New 0 / Needs-Context 1 / Awaiting-Approval 4 / Done 1. Isolation re-verified: demo=6 real, Arthur=519 real, 0 cross-tenant leak (`welcome@zoho.com` appears in both only as a coincidental external sender).
...
Channel scope (locked 2026-07-09): REAL & live = Gmail, 2nd-email (IMAP). Built-but-credential/validation-gated (real code, awaiting a working session/creds) = **Telegram (MTProto)**, X DMs, Twilio SMS/WhatsApp. DROPPED = LinkedIn (no personal-messaging API).

**Telegram status (2026-07-09):** connector + resolver + guided UI connect + login script all BUILT and wired. Bringing a live session up is blocked by **Telegram's own login validation**, not our code: MTProto logins to a personal account require the account holder to approve the new session from their existing Telegram app, and login codes relayed to a third-party client are invalidated server-side (anti-phishing). Verified the hard way — 3 relayed-code attempts all returned `PhoneCodeExpiredError` even on a single live connection with instant relay (api_id `31761379`; friend's account). So Telegram sits in the built + bring-your-own-session band, same as X/SMS/WhatsApp — real and ready, activates once the account owner completes Telegram's in-app approval. **PR/email framing (Arthur, keep verbatim intent):** "Telegram connector built and integrated; live activation gated by Telegram's server-side login validation (in-app session approval + relayed-code invalidation) — not a missing capability."
...
- 2026-07-09 — **Retiring fixture CODE does not retire fixture DATA.** The multi-tenant refactor deleted every fixture connector/corpus/seed script from the repo and I logged "all fakes retired" — but the demo tenant's *store* still held 137 persona rows (`raw_ref` `data/fixtures/%` and `seed:%`) across gmail/x/sms/whatsapp/linkedin/email. So the grader logging into `demo@meridianlabs.io` would have seen the Jordan-Reeve corpus on channels that aren't even connected — the exact "fixtures reading back as the product" confusion, one layer down. Fix: purge by provenance, keeping only real rows (`raw_ref like 'imap:%'`), scoped to the tenant's `owner_id`; then re-verify from a live store query, not from the code diff. Rule: **when you retire a data source, delete its rows too — "reality is the store, not the repo" (CLAUDE.md reality hierarchy) cuts both ways.**
